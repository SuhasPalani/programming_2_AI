...
def forward_checking_mrv_solve(grid):
    ...
    def select_unassigned_variable(grid):
        # Implement your logic for selecting the most constrained variable
        pass

    def order_domain_values(grid, var):
        # Implement your logic for ordering domain values
        pass

    def inference(grid, var, value):
        # Implement your logic for forward checking
        pass
    ...
    def backtrack():
        ...
        if row is None:
            return True  # Puzzle is solved
        var = select_unassigned_variable(grid)
        for value in order_domain_values(grid, var):
            if is_valid_move(grid, row, col, value):
                grid[row][col] = value
                nodes_explored += 1  # Increment nodes_explored
                inferences = inference(grid, var, value)
...
def main():
    ...
    mode = int(sys.argv[1])  # Convert the mode to an integer
    filename = sys.argv[2]
    ...
    if mode == 1:
        ...
        solver = forward_checking_mrv_solve
    else:
    ...
    try:
        ...
        nodes_explored = solver(grid)
        search_time = time.time() - start_time

        if not nodes_explored:
            ...
            if is_valid_solution(grid):
                ...
                if mode in (1, 2, 3):
                    ...
                    print(f"\nSolution saved to {solution_filename}")
            else:
